...
import { Upload, Trash2, Plus, X, Check, Shield, Loader2 } from 'lucide-react'
...
export default function AdminPage() {
  ...
  async function fetchPapers(db: ReturnType<typeof createClient>) {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const db = supabase as any
    const fileName = `${Date.now()}-${form.file.name.replace(/\s+/g, '_')}`
    ...
    const slug = `${form.subject.toLowerCase().replace(/\s+/g, '-')}-${form.exam_type.toLowerCase().replace(/[^a-z]/g, '')}-${form.year}-${form.branch.toLowerCase()}-s${form.semester}-${Date.now()}`
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {toast && (
        <div className="fixed top-20 right-4 bg-[#1e2d3d] text-white px-4 py-3 rounded-xl shadow-lg text-sm z-50 flex items-center gap-2">
          <Check className="w-4 h-4 text-teal-400 flex-shrink-0" /> {toast}
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2 rounded-xl border border-[#E8E4DC] text-sm font-medium text-slate-600 hover:bg-[#FAF7F2] disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2 rounded-xl bg-rose-500 text-white text-sm font-semibold hover:bg-rose-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">{loadingPapers ? 'Loading...' : `${papers.length} papers in archive`}</p>
          </div>
        </div>
        <button
        ...
        <div className="bg-white rounded-2xl border border-[#E8E4DC] p-6 shadow-sm mb-8">
          ...
          <form onSubmit={handleUpload} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="sm:col-span-2 lg:col-span-3">
              <button
                type="submit"
                disabled={uploading || !form.subject.trim() || !form.file}
                className="flex items-center gap-2 bg-teal-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-teal-700 disabled:opacity-60 transition-colors"
              >
      ...
      <div className="bg-white rounded-2xl border border-[#E8E4DC] shadow-sm overflow-hidden">
        {loadingPapers ? (
          <div className="flex items-center justify-center py-16">
            ...
            <span className="ml-2 text-slate-500 text-sm">Loading papers...</span>
          </div>
        ) : papers.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-slate-400 text-sm">No papers yet. Upload one above.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#E8E4DC]">
                {papers.map((paper) => (
                  <tr key={paper.id} className="hover:bg-[#FAF7F2] transition-colors">
                    <td className="px-5 py-3.5 font-medium text-[#1e2d3d] max-w-[200px] truncate" title={paper.subject}>{paper.subject}</td>
